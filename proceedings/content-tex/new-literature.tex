...
\cite{baumgartner2017-IEEETransMedImag} proposed SonoNet which is a VGG-based arquitecture, SonoNet64 used the same first 13 layers of VGG16,
...
applied four feature extraction networks couple with batchnormalisation and soft proposal layer (VGG13-SP, VGG16-SP, ResNet18-SP, ResNet34-SP) being ResNet18-SP the best performing network with avarage accuracy over six classes of fetal US views (0.912). 
...
\cite{Al-Dhabyani2019-IJACSA} applied AlexNet and transfer learning based arquitectures (VGG16, Inception, ResNet, NASNet) without augmentation and with three augmentation techniques to perform tumor classification of breast ultrasound imaging. Authors stated that transfer learning NASNet presented the best performance with 99\% with BUSI+B datasets with DAGAN augmentation. 
\cite{xie2020-physics-in-medicine-biology} proposed a dual-sampling convolutional neural network (DSCNN) for US image breast cancer classification, being DSCNN more efficent than AlexNet, VGG16, ResNet18, GoogleNet and EfficientNet. 
\cite{snider2022-ScientificReports} reported summaries of CNN heuristics to detect shrapnel in US images. 
Authors presented summaries of model performance for layer activators, 2D CNN layer arquitectures, model optimisers dense nodes, and the effect of image augmentation and dropout rate and epoch number. 
